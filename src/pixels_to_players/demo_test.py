...
class PixelsToPlayersDemo:
    ...
    def test_webcam_basic(self) -> bool:
        ...
        try:
            with WebcamClient(self.demo_config["webcam"]) as cam:
                ...
                while True:
                    frame = cam.snapshot(
                        processors=[P.flip_horizontal, P.draw_facemesh])
    ...
    def test_calibration(self) -> bool:
        ...
        try:
            ...
            print(
                "You will see 5 calibration points. Look at each one for 2 seconds.")
            ...
            with WebcamClient(self.demo_config["webcam"]) as cam:
                ...
                for i, (normalized_x, normalized_y) in enumerate(CALIB_POINTS):
                    print(
                        f"Look at point {i + 1}/5: ({normalized_x:.1f}, {normalized_y:.1f})")
                    start = time.time()
                    samples = []
                    ...
                    while time.time() - start < 2.0:  # 2 seconds per point for demo
                        ...
                        if iris_center:
                            ...
                            cv2.circle(frame, (
                                int(iris_center[0]), int(iris_center[1])), 5,
                                       (0, 255, 255), -1)
                    ...
                    if samples:
                        ...
                        print(
                            f"Captured point {i + 1}: ({normalized_x}, {normalized_y}) -> eye {avg}")
    ...
    def test_screen_recording(self) -> bool:
        ...
        try:
            ...
            # Version 2 returns tuple (video_path, features_data)
            video_path, features_data = recorder.record(duration_seconds=5.0)

            if video_path.exists():
                file_size = video_path.stat().st_size
                print(
                    f"Screen recording completed: {video_path.name} ({file_size} bytes)")
                return True
            else:
    ...
    def test_integrated_demo(self) -> bool:
        ...
        try:
            # Start screen recording - Version 2 returns tuple
            recorder = ScreenRecorder(self.demo_config["screen_recording"])
            video_path, initial_features_data = recorder.start()
                ...
                while time.time() - start_time < 8:  # 8 second demo
                    ...
                    if iris_center:
                        ...
                        cv2.circle(frame,
                                   ...
                                   5, (0, 255, 0), -1)
                        cv2.putText(frame,
                                    f"Gaze: ({iris_center[0]:.1f}, {iris_center[1]:.1f})",
                                    (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7,
                                    (0, 255, 0), 2)

                    cv2.putText(frame,
                                "Integrated Demo - Screen Recording Active",
            ...
            # Stop screen recording - Version 2 returns features_data
            final_features_data = recorder.stop()
    ...
    def run_demo(self) -> None:
        ...
        print(
            "This demo needs camera and recording permissions so please start any test, allow permissions, and restart if this is your first time running the demo")
        print(
            "Follow along in the terminal, some instructions might have you tab in to the application that opens")
